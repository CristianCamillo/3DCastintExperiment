...
{
	// Cristian Camillo, 2019-11-11
	// Copy-paste this class together with "Bitmap"
	...
	public SimpleEngine(int width, int height, String title, int FPSCap, boolean showFPS, boolean fullscreen)
	{		
		this.width = width;
		this.height = height;
		setTitle(title);
	...
	private boolean toggle(int code, boolean[] baseArray, boolean[] toggleArray)
	{
		if(baseArray[code])
		{
			if(!toggleArray[code])
			{
				synchronized(SimpleEngine.class)
				{
	...
	public final Bitmap getFrameBuffer() 		{ return frameBuffer; }
	public final Graphics2D getGraphics2D()		{ return frameBuffer.getGraphics2D(); }
	...
	{	
		...
		frameBuffer = new Bitmap(getWidth(), getHeight());
